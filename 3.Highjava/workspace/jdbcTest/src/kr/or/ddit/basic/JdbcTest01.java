...
/*
 * 	- 데이터베이스 처리 순서
 * 	 1. 드라이버 로딩 : 라이브러리를 사용할 수 있도록 메로리로 읽어 들이는 작업
 * 		(JDBC API 버전 4이상에서는 getConnection()메서드에서 자동으로 로딩해주기 때문에 생략할 수 있다.(그렇지만 생략하고 않고 사용할 예정))
 * 		Class.forName("oracle.jdbc.driver.OracleDriver);
 * 
 * 	 2. DB에 접속하기 : 접속이 완료되면 Connection객체가 반환된다.(DB와 연결할때 필요한 정보를 가지고 있는 객체)
 ...
 * 		2) SQL문이 select문이 아닐 경우(insert, update, delete 등) : SQL문의 실행 결과가 정수값이 반환된다. (정수값은 보통 실행에 성공한 레코드 수를 말한다.)
 * 
 ...
 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			// rs.next() : rs객체의 데이터를 가리키는 포인터를 다음번째 레코드로 이동시키고, 그 곳에 데이터가 있으면 true, 없으면 false를 반환한다.
			while(rs.next()) {
				// 현재 rs객체의 포인터가 기리키는 곳의 자료를 가져오는 법
