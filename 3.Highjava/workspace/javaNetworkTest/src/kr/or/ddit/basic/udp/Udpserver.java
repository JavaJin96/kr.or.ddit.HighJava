...
/*
 * 	UDP방식 : 비연결 지향, 신뢰성이 없다. 데이터가 순서대로 도착한다는 보장이 없다. 그렇지만 TCP방식보다는 빠르다.
 * 
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 데이터를 수신한다. : receive()메서드 이용 (파라미터에 수신용 패킷을 넣어준다.)
				// : 이 메서드는 데이터가 올 때까지 기다린다.
				// : 수신된 데이터의 패킷 정보는 인수로 지정한 수신용 패킷객체변수에 저장된다. 
				...
				// 상대방에게 메시지 보내기 (수신받은 메세지를 그대로 송신)
				
				// 송신할 메세지를 byte형 배열로 변환한다.
				byte[] sendMsg = msg.getBytes("utf-8");
				
				// 송신용 패킷객체 생성
				// : 전송할 데이터가 저장된 byte형 배열, 전송할 자료의 길이(배열의 길이),상대방 주소 정보, 상대방 포트번호를 지정해서 생성한다.
