...
public class SetTest {
	...
	public static void main(String[] args) {
		...
		// Set에 데이터를 추가할 때는 add()메서드를 사용한다.
		hs1.add("DD");
		hs1.add("AA");
		hs1.add(2);
		hs1.add("CC");
		...
		System.out.println("set 사이즈 : " + hs1.size());
		...
		// Set의 데이터를 수정하기
		// 수정하는 메서드가 따로 없기 때문에 해당 자료를 삭제하고 다시 추가하는 방법을 사용한다.
		
		// 삭제하는 메서드 : remove(삭제할 자료)  
		// 반환값 : 삭제가 성공하면 true 실패하면 false
		// 삭제하는 메서드 : clear() 전체삭제
		...
		hs1.add("EE");
		System.out.println("입력후 Set : " + hs1);
...
//		System.out.println("set 데이터 : " + hs1);
		...
		/*
		 *  Set 의 데이터는 순서 (index)가 없기 때문에 List처럼 index로 데이터를 하나씩 불러올 수 없다.
		...
		// Iterator의 hasNext()메서드 : Iterator의 포인터가 가리키는 곳의 다음번째에 데이터가 있는지 검사
		while(it.hasNext()) {
			// Iterator의 next()메서드 : Iterator의 포인터를 다음 데이터 위치로 이동시키고 이동한 위치의 데이터를 반환한다.
		...
		System.out.println("당첨된 사람 : " + testSet);
