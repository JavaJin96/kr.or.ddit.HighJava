...
<%
List<temp_StudentInfoVO> slist = (List<temp_StudentInfoVO>)request.getAttribute("slist"); 
%>
<script>
cnt = 1
$(function(){
...
<header>
 ...
 <%for(int i = 0 ; i < slist.size() ; i++){
	 if(slist.get(i).getMem_sort_num().equals("1")){%>
      <tr>
        <td><script type="text/javascript">document.write(cnt++)</script></td>
        <td><%=slist.get(i).getMem_name()%></td>
        <td><%=slist.get(i).getDep_name()%></td>
        <td><%=slist.get(i).getMem_id()%></td>
        <td><%=slist.get(i).getStart_date()%></td>
        <td><%=slist.get(i).getStatus()%></td>
      </tr>
 <%}}%>
    </tbody>
  </table>
