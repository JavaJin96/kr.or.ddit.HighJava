...
public class CookieRead extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 저장된 Cookie정보 읽어오기
		// 1. 전체 쿠키 정보를 request객체의 getCookies()메서드를 이용하여 가져온다. : 가져온 쿠키 정보들은 배열에 저장된다.
		...
		if(cookieArr == null || cookieArr.length == 0) {
			...
			for(Cookie ck : cookieArr) {
				String cookieName = ck.getName();	// '쿠키변수' 가져오기
				String cookieValue = ck.getValue();	// '쿠키값' 가져오기
				out.println("쿠키변수 : " + cookieName + "<br>" + "쿠키값 : " + cookieValue + "<br><hr>");
			}
		}
