...
public class SessionRead extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. Session객체를 생성하거나 현재 Session객체 객체 가져오기
		...
		if(sessionValue==null) {
			out.println("<h3> testSession의 Session값이 없습니다. </h3>");			
		}else {
		...
		// 가장 최근에 세션에 접근한 시간 : 생성시간 : 1970년 1월 1일 부터 경과한 시간(밀리세컨드 단위)
		out.println("세션 최근 접근 시간 : " + session.getLastAccessedTime() + "<br>");
		
		// 세션의 유효시간 : 초 단위 (세션이 사용된 후 자동으로 삭제될 때까지의 시간
		// 유효시간 설정 방법 : session.setMaxInactiveInterval(시간/초)
		
		// web.xml 에서 유효시간 설정방법
		/*
		 * 	<session-config>
