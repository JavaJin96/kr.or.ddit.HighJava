...
public class RequestTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// GET방식의 인코딩은 server.xml 64번째줄의 URLEncoding="utf-8" 를 기술했기 때문이다.
		...
		// 							   : 파라미터명은 <form>태그 안에 데이터를 구성하는 태그들의 name속성 값이다.
		
		String userName = request.getParameter("username");
		String job = request.getParameter("job");
		
		
		// 2) getParameterValues("파라미터명") : 파라미터명(name속성)이 같은 것이 여러개 일 경우에 사용한다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// doPost메서드에서 doGet메서드가 호출된 이유 : 한번에 처리하기 위함(doGet에 기술해도 post로 처리하기 위함)
