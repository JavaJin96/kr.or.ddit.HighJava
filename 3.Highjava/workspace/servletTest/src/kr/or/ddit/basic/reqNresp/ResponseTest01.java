...
public class ResponseTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * 	 : 클라이언트의 URL주소는 처음 요청할 때의 값이 바뀌지 않으며, 
		 * 	   서버 내부에서만 접근이 가능하다.
		 * 
		 * 	 : 처음 요청을 받은 서블릿과 제어를 넘겨받은 서블릿이나 JSP에서는 
		 *   Request객체와 Response객체를 공유해서 같이 사용할 수 있다.
		 *   
		 *   : 보통 이동되는 페이지로 값을 넘기려면 Request객체의 setAttribute()메서드로 데이터를
		 *   세팅하여 보내고, 받는 쪽에서는 getAttribute()메서드로 데이터를 읽어서 사용할 수 있다. 
		 */
		 ...
		 *  	키값 : 반드시 문자열로 지정한다.
		 *  	데이터값 : 자바의 모든 데이터 종류 사용 가능
		 */
		request.setAttribute("tel", "010-1234-5678");
		
		// 파라미터 부분 : 이동할 문서명(ContextPath이후의 경로를 기술한다.)
