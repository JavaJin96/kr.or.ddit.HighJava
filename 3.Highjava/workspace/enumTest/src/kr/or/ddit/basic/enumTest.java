 ...
 *  또 클래스 안에 내부 클래스처럼 만들 수 있다. 
 * 
 * 
 *  - 열거형의 속성 및 메서드 
 *  * name() : 열거형 상수의 이름을 문자열로 반환한다.
 *  * ordinal() : 열거형 상수가 정의된 순서값(index값)을 반환한다.
 *  * valueOf("열거형 상수명") : 지정된 열거형에서 '열거형 상수형'과 일치하는 열거형 상수를 반환한다.
 ...
 *  
 *  
 *  		// 매개변수에 주어지는 '변수명' 들은 '값들'과 개수가 같고, 각각의 '값들'과 자료형이 맞아야 한다.
 *  		private 열거형이름 (자료형이름 변수명, ....) {
 *				  위에 선언된 '값들'이 저장될 변수에 대한 초기화 작업을 수행한다. 
 *  
 *  		}
 *  
 *  		//위에 선언된 '값들'이 저장될 변수를 외부에서 사용할 수 있도록 getter메서드를 작성한다.
 *  		....
 *  
 *  }
 * 
...
public class enumTest {
	...
	public enum Season{
		...
		// 생성자
		Season(String month, int data) { // private Season(String month, int data)
			span = month;
			num = data;
		}
...
//		}
		...
		// switch 문에서 열거형 비교하기 : case문에 지정하는 값은 열거형이름을 생략한 상수명만 기술한다.
		switch(mycnt) {
		case ONE:
		...
		for(Season time : Season.values()) {
			System.out.println(time.name() + " == " + time + time.getNum() 
							+ "번째 상수값 : " + time.getSpan() );
		}
